...
@property (nonatomic, strong) UIButton *closeButton, *forgetBtn;
...
@property (nonatomic, strong) UILabel *tipLabel ,* titleLab;            // 提示文字
...
- (void)setupUI{
    ...
    [self.backView addSubview:self.forgetBtn];
    [self.backView addSubview:self.tipLabel];
    
    self.coverView.frame = self.bounds;
    self.closeButton.frame = CGRectMake(kScreenW - 50, 10, 40, 30);
...
- (void)showInView:(UIView *)view{
    ...
    self.backView.frame = CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, 440);
    [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionTransitionNone animations:^{
        self.backView.frame = CGRectMake(0, SCREEN_HEIGHT-440, SCREEN_WIDTH, 440);
...
- (UILabel *)titleLab {
    if (!_titleLab) {
        _titleLab = [[UILabel alloc] initWithFrame:FRAME(kScreenW/4, 10, kScreenW/2, 30  )];
        _titleLab.font = SourceHanSansCNMedium(15);
        _titleLab.text = @"请输入支付密码";
